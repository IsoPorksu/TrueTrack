...
def print_vehicle_table(): # PRINT FUNCTION!
    ...
    print(" Car |  Now -> Next | Destination")
    print(" ----|--------------|------------")
    ...
    for vehicle_number, station in sorted.items():
        ...
        elif (station.endswith("1") or station. endswith("2")) or len(station) == 4:
            #print("TRUE") # DEBUG
            if next == "":
...
def on_message(client, userdata, message):
    ...
    # Check lat and long are legit coords
    try:
        latitude = float(latitude)
        longitude = float(longitude)
    except ValueError:
        print(f"Invalid latitude or longitude for vehicle {vehicle_number}: {latitude}, {longitude}")
        return
